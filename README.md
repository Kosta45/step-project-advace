...
## - у правому кутку – Кнопка "Вхід"(login). Після успішної авторизації вона має змінюватися на Кнопку "Створити візит"(create visit).

## 2. Під Header – форма для фільтрації візитів. У цій формі має бути 3 поля:

## - пошук за заголовком/вмістом візиту (search by client name and purpose description)

## - пошук за статусом (Open/Done) (візит пройшов чи ще ні)

## - терміновість візиту (High, Normal, Low)

## - добавили пошук по лікарям (cardiologist, therapist, dentist)

## - добавили керпку фільтрувати при натиску фільтруються картки по вибраним данам
...
## - Для AJAX запитів можна використовувати `fetch` або `axios`. (ці дані знаходяться в src/constants/api.js)
...
- Після вибору лікаря зі списку, під ним повинні з'явитися поля для запису до цього лікаря. Декілька полів є однаковими для всіх трьох докторів: **(цю логіку трохи змінили)**

## - мета візиту (purpose)

## - короткий опис візиту (description)

## - дропдаун - терміновість (звичайна, пріоритетна, невідкладна) (Urgency of the visit --> Urgency, normal, priority, urgent)
...
## - звичайний тиск (Normal pressure)

## - Індекс маси тіла (Body mass index)

## - перенесені захворювання серцево-судинної системи (Past cardiovascular diseases)

## - вік (Age)

## - Якщо вибрано опцію **Стоматолог**, додатково необхідно заповнити:

## - дата останнього відвідування (Date of last visit)

## - Якщо вибрано опцію Терапевт, додатково необхідно заповнити:

## - вік (Age)

## - Кнопка `Створити`(Create). При натисканні на кнопку надсилається AJAX запит на відповідну адресу, і якщо у відповіді надійшла інформація про новостворену картку - створюється картка у Дошці візитів на сторінці, модальне вікно закривається.
...
## - Іконка з хрестиком у верхньому правому кутку, при натисканні на яку картку буде видалено(це змінили буде тільки скасувати)
